...
if (isset($_GET['id_transaksi'])) {
  ...
  $productieorder =  $_GET['id_transaksi'];
  $updateStatus = "UPDATE transfer_bank SET status = 'berhasil' WHERE id_transaksi=".$productieorder."";
  $query2 = $koneksi->query($updateStatus);
  }
...
function cari($keyword) {
    $query = "SELECT * FROM transfer_bank LEFT JOIN buku USING (kode_buku) WHERE username  LIKE '%$keyword%'
...
$dimas = query("SELECT * FROM transfer_bank LEFT JOIN buku USING (kode_buku) ORDER BY id_transaksi DESC");
...
<table class="table table-striped table-bordered">
        ...
        <th scope="col">NO</th>
            ...
            <th scope="col">Bank pelanggan</th>
            <th scope="col">Rekening pelanggan</th>
    ...
    <?php foreach( $dimas as $row) : ?>
       <tr>
        ...
        <td><a class="btn btn-success" href="data-pembayaran-admin.php?id_transaksi=<?= $row["id_transaksi"]; ?>"><?= $row["status"]; ?></a></td>
        <td><?= $row["dari_bank"]; ?></td> 
        <td><?= $row["dari_rekening"]; ?></td>
        <td><?= $row["nama_pemilik"]; ?></td>
        <td><img src="../foto bukti/<?php echo $row["foto_bukti"]; ?>" width="50"></td> 
      </tr> 
